...
class DiffStore:
    ...
    @staticmethod
    def _set_diffItem(key: str, data: DiffItem):
        if not DiffStore.keyringItem:
            raise Exception("DiffStore not initialized")
    ...
    @staticmethod
    def get(
        key: str, version: str = "latest", _data: DiffItem | None = None
    ) -> str | None:
        if _data:
            data = _data
        else:
